...
show_month() { 
	...
	option=$(echo "<<
$(cal -v $mnth $year)
>>"| rofi -dmenu -p 'Calendar' -location 3 -yoffset 25 -lines 12 -selected-row $posn -theme ~/.config/rofi/themes/flat-orange.rasi -theme-str 'window {width: 320px; lines: 12;}')
	case $option in
        ">>")
			mnth=$((10#$mnth + 1))
			if [[ $mnth == '13' ]] ; then
